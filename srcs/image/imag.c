...
/*   Updated: 2021/07/27 18:38:34 by xuwang           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "so_long.h"
/* Put a pixel on the screen */
void	pixel_put_color(t_vars *vars, int x, int y,
		unsigned int color)
{
	char	*dst;
...
{
	const double	pixe_x = ((double)1000 / vars->map_x_len);
	const double	pixe_y = ((double)1000 / vars->map_y_len);
	char			*addr;
	double			x;
	double			y;
	...
	while (y < pixe_y)
	{
		...
		while (x < pixe_x)
		{
			...
			addr += (size_t)((y / pixe_y)
					* vars->tex[i].height) * vars->tex[i].line_len;
			addr += (size_t)((x / pixe_x) * vars->tex[i].width) * 4;
			pixel_put_color(vars, x + (x_index * pixe_x),
				y + (y_index * pixe_y), *(unsigned int *)addr);
			x++;
		}
		y++;
	}
